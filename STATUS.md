...
All ten build steps are complete, the app runs, **249 tests pass**, and both pipelines are
...
by any of this UI/report work; none of it is a reason to avoid touching `app/templates/`,
`app/static/`, or `app/report.py` — that's exactly what this phase is for.

**UI/report work landed so far, 2026-08-06 through 2026-08-10:** a series of disclaimer-
copy trims (CLAUDE.md §18, passes one through three — the triage note, lief caveat, MITRE
disclaimer, and the Appendix's library-version table all removed from user-facing pages,
none of it a change to underlying behaviour); registration's email field made mandatory,
server-side (`RegisterForm.email` switched from `Optional()` to `DataRequired()` — four of
the six existing accounts had no email set and were left alone, only new registrations are
affected); and a fourth §18 pass adding real content rather than removing it — an optional
case/investigation reference field on upload, PDF page numbers and a running SHA-256
footer, resolved parent-process names in the memory evidence section (new ground: reads
`pslist`'s `ImageFileName` column for the first time in this codebase, verified against
the installed Volatility 3 source directly), a "report generated by" line distinct from
the job's owner, and matching visual dividers on both the PDF and the job-detail page. Full
...
## Build state: all ten steps complete, 249 tests passing

```
.venv\Scripts\python -m pytest tests -q      ->  249 passed
```
